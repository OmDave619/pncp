...
class BakeryLock {
private:
    vector<bool> flag;  // true if ith thread wants to or is currently in enter critical section
    vector<int> ticket; // current ticket assigned to each thread 
public:
    BakeryLock(int n) {
        flag = vector<bool>(n, false);  // initially no thread wants to enter critical section
        ticket = vector<int>(n, 0);     // initially all tickets are 0
    }

    void lock(int id) {
        //converting 1 based thread ids into 0 based ids
        id--;
        flag[id] = true;    //thread i wants to enter critical section
        ticket[id] = *max_element(ticket.begin(), ticket.end()) + 1; //assigning new ticket to ith thread
        while(1) {
            bool conflict = false;
            for (int k = 0; k < n; k++) {
                if (k == id) continue;
                // wait if there is other is there with smaller ticket or if tickets are same then check for lower id
                if(flag[k] && (ticket[k]<ticket[id] || (ticket[k]==ticket[id] && k<id))) {
                    conflict = true;         
                    break;           
                }
            }   
    ...
    void unlock(int id) {
        //converting 1 based thread ids into 0 based ids
        id--;
        flag[id] = false; //thread i no longer wants to enter critical section
    }
};
