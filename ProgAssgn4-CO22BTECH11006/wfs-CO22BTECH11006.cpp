...
void* writer(void* arg) {
    ...
    WaitFreeSnapshot<int>* snap = args->snap;
...
int main() {
    ...
    FILE* output = fopen("./output_wfs.txt", "w");
    if (output == NULL) {
    ...
    for(int i = 0; i < ns; i++) {
        ...
        snapshot_args[i].snap = &WFsnapshot;
    ...
    std::sort(all_logs.begin(), all_logs.end(), compareLogEntries);
